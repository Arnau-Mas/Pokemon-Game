...
export function PokemonFighter({pokemon, face, life}){
    ...
    <div className="pokemonContainer">
        ...
        <div style={{width:"70%", display:"grid", gridTemplateColumns:"2", gap:"0.5rem", justifyContent:"center", marginBottom:"1rem"}}>
            <button className="moveContainer__move" style={{gridColumn:"1/2"}}>{moves[0].move.name}</button>
            <button className="moveContainer__move" style={{gridColumn:"2/2"}}>{moves[1].move.name}</button>
            <button className="moveContainer__move" style={{gridColumn:"1/2"}}>{moves[2].move.name}</button>
            <button className="moveContainer__move" style={{gridColumn:"2/2"}}>{moves[3].move.name}</button>
        </div>
    </div>
    )
}